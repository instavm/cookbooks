...
Once readiness completes, open the share URL InstaVM prints. You will see a code-server login screen; enter the `PASSWORD` you supplied.
...
instavm deploy . \
  --port 8080 \
  --health-path /healthz \
  --start-command 'code-server --bind-addr 0.0.0.0:8080 --auth password --disable-telemetry --disable-update-check /workspace' \
