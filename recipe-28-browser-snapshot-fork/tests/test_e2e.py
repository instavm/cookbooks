...
def test_e2e_offline_happy_path(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("INSTAVM_FORK_MOCK", "1")
    resp = client.post("/fork", json={"tasks": ["alpha", "beta"]})
    assert resp.status_code == 200


@pytest.mark.skipif(not secret_available("INSTAVM_API_KEY"), reason="INSTAVM_API_KEY missing")
def test_e2e_live_optional():
    """Run manually against a deployed VM when vault + keys are configured."""
    assert secret_available("INSTAVM_API_KEY")
